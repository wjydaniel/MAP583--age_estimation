...
def train(train_loader, model, model_1, criterion1,criterion2, optimizer, epoch, device):
    ...
    with tqdm(train_loader) as _tqdm:
        for x, y in _tqdm:
            ...
            op = torch.mm(op, torch.reshape(torch.arange(0,101,dtype=torch.float32), (101,1)).to(device))
            #yy = yy - op.round().squeeze().int()
            op = op.squeeze()
            y = y - op.round().int()
            y = torch.clamp(y , min=-20,max=20)

            # compute output
            prob_residual = model(x)
            residual = F.softmax(prob_residual,dim=-1)
            residual = torch.mm(residual, torch.reshape(torch.arange(-50, 51, dtype=torch.float32), (101, 1)).to(device))
            residual = residual.squeeze()


            # calc loss
            loss = criterion1(residual, y) + criterion2(prob_residual,y+50)
            cur_loss = loss.item()
...
def validate(validate_loader, model, model_1, criterion1,criterion2, epoch, device):
    model.eval()
    loss_monitor = AverageMeter()
    accuracy_monitor = AverageMeter()
    preds = []
    ...
    with torch.no_grad():
        with tqdm(validate_loader) as _tqdm:
            for i, (x, y) in enumerate(_tqdm):
                ...
                prob_residual = model(x)
                residual = F.softmax(prob_residual, dim=-1)
                residual = torch.mm(residual,
                                    torch.reshape(torch.arange(-50, 51, dtype=torch.float32), (101, 1)).to(device))
                residual = residual.squeeze()

                preds.append(residual.cpu().numpy()+op.cpu().numpy())
                gt.append(yy.cpu().numpy())
...
def main():
    ...
    for epoch in range(start_epoch, cfg.TRAIN.EPOCHS):
        # train
        train_loss, train_acc = train(train_loader, model,model_1, criterion, criterion2, optimizer, epoch, device)

        # validate
        val_loss, val_acc, val_mae = validate(val_loader, model,model_1, criterion, criterion2, epoch, device)
