...
def train(train_loader, model, criterion1,criterion2, optimizer, epoch, device):
    model.train()
    ...
    with tqdm(train_loader) as _tqdm:
        for x, y in _tqdm:
            ...
            # compute output
            op = model(x)
            outputs = F.softmax(op,dim=-1)
            outputs = torch.mm(outputs, torch.reshape(torch.arange(0,101,dtype=torch.float32), (101,1)).to(device))
            outputs = outputs.squeeze()

            # calc loss
            loss = criterion1(outputs, y) + criterion2(op,yy)
            cur_loss = loss.item()
...
def main():
    ...
    for epoch in range(start_epoch, cfg.TRAIN.EPOCHS):
        # train
        train_loss, train_acc = train(train_loader, model, criterion, criterion2, optimizer, epoch, device)
